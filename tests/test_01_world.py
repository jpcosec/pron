...
def test_store_passes_integrity_and_lists_exactly_the_declared_models(world: World):
    assert sldb_main(["stores", "check", "--store", str(world.store.sp), "--pythonpath", world.store.pythonpath]) == 0
    assert set(world.model_names()) == {"Client", "Table", "Reservation", "State", "RelationTypeDoc", "RelationDoc", "AnchorDoc", "ProjectionDoc", "MoveDoc"}
...
def test_schema_and_family_come_from_the_store(world: World):
    ...
    assert fields["status"]["kind"] == "enum" and fields["status"]["enum"] == ["pending", "confirmed", "seated", "cancelled"]
...
def test_addresses_and_fields_read_by_address(world: World):
    ...
    assert sorted(world.store.find("st.{Table}", 'zone = "terrace"')) == ["st.{Table}.table-12", "st.{Table}.table-14", "st.{Table}.table-20"]
...
def test_alias_with_steps_round_trips(world: World):
    ...
    assert d.payload["steps"][1] == {"do": "assert", "relation": "booked_by", "source": "$created", "target": "$referent:Client"}
...
def test_graph_is_built_fresh_and_typed(world: World):
    ...
    edges = world.graph.edges_from("sldb://document/Reservation:reservation-2026-09-11-luis-soto")
...
def test_hash_mundo_ignores_the_ledger_but_sees_a_schema_change(world: World):
    ...
    world.store.create("MoveDoc", "move-test-1", {"id": "move-test-1", "at": "2026-09-09T00:00:00Z", "speaker": "", "outcome": "unico", "state_before": "libre",
                                                    "state_after": "libre", "hash_before": before, "hash_after": before, "refers_to": "", "sentence": "x", "record": {"a": 1}},
