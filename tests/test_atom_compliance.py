...
def test_atom_sexpr_py_is_the_dependency_free_meaning_kernel():
    ...
    assert not project_imports, f"sexpr.py must import nothing from the project: {project_imports}"
...
def test_atom_resolution_outcomes_are_typed_values_not_exceptions():
    ...
    class FakeBridge:
        ...
        def documents_of_model(self, m): return self._d
...
def test_atom_resolution_py_implements_the_selector_cascade():
    ...
    from knowledge.core.resolution import resolve_noun
    from knowledge.core.results import Resolved
...
def test_atom_session_expires_by_ttl_and_store_hash(tmp_path):
    ...
    s.save("(check _)", ["a"])
    data = s.path.read_text().replace(
        str(int(time.time())), str(int(time.time() - mod.TTL_SECONDS - 60)), 1)
    import json as j
...
def test_atom_bridges_are_the_only_doors_to_sldb_and_kgdb():
    ...
    for py in (ROOT / "src/knowledge").rglob("*.py"):
        ...
        for node in ast.walk(tree):
            ...
            if module.startswith("sldb") or module.startswith("kgdb") or any(
                n.startswith(("sldb", "kgdb")) for n in names
            ):
...
def test_atom_one_file_one_component_one_motive():
    ...
    expected = {
        "core/sexpr.py", "core/anchors.py", "core/resolution.py",
        "core/session.py", "core/evaluator.py", "core/results.py",
        "ops/read.py", "bridges/sldb_bridge.py", "bridges/kgdb_bridge.py",
        "infra/projector.py", "cli/surface.py", "cli/render.py", "cli/main.py",
    }
...
def test_atom_unanchored_symbols_fail_with_an_explicit_semantic_error():
    ...
    class EmptyBridge:
        def documents_of_model(self, m): return []
...
def test_atom_anchor_ref_is_a_typed_string_with_a_scheme_per_kind():
    ...
    ok = [
        ("model", "model:UserDoc"), ("doc", "doc:atom-x"),
        ("relation", "edge:tagged_as"), ("relation", "edge:follows:in"),
        ("operation", "op:check"), ("projection", "fields:id,title"),
        ("projection", "view:summary"), ("expr", "expr:(rel tagged _)"),
    ]
    for kind, ref in ok:
...
def test_atom_render_py_maps_result_values_to_output_and_exit_codes():
    ...
    from knowledge.core.results import (
        Ambiguous, Missing, OperationResult, SemanticError,
    )
...
def test_atom_every_read_response_carries_refs_for_auditability():
    ...
    for fn, arg in ((read.check, doc), (read.return_, doc),
...
def test_atom_kgdb_bridge_guards_staleness(tmp_path):
    ...
    graph.write_text(json.dumps({"nodes": [
        {"id": "n1", "schema": {"source": {"store": {"hash_a": "old-hash"}}}}
    ]}))
