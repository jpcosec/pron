...
def test_a_fragment_that_fits_the_hole_corrects_the_missing_turn(session: Session):
    assert session.turn("create a client named Ana Rojas, phone 9 5555 1234").outcome == "unico"
    ...
    assert r.record["corrects"]["move"] == missing_move and "terrace" in r.record["corrects"]["sentence"]
...
def test_the_created_object_is_named_once_its_relations_exist(session: Session):
    last = session.ledger.get(session.ledger.last_with_write("jp")["id"])
    text = last["record"]["trace"][-1] if False else None  # the answer is in the response, not the trace
...
def test_a_coordinated_move_is_validated_whole_before_the_first_write(session: Session):
    ...
    assert after["party_size"] == before["party_size"] == 6 and after["status"] == "pending"
...
def test_undo_does_not_touch_a_document_changed_since_the_move(session: Session):
    ...
    session.world.store.update_field("Reservation", RES, "party_size", 7)   # someone else, outside pron
...
def test_a_compose_alias_respects_the_relation_permissions(world: World):
    proj = dict(world.projection("all"), name="creator", actions=["create"],
                relations=[{"name": "booked_by", "mode": "read"}, {"name": "assigned_to", "mode": "read"}, {"name": "transitions_to", "mode": "read"}])
    world.store.create("ProjectionDoc", "projection-creator", proj, world.root / "knowledge" / "projections" / "creator.md")
...
def test_the_projection_cuts_aliases_whose_target_is_outside_it(world: World):
    proj = dict(world.projection("all"), name="clients", models=["Client"], relations=[], actions=[])
    world.store.create("ProjectionDoc", "projection-clients", proj, world.root / "knowledge" / "projections" / "clients.md")
...
def test_hash_mundo_is_read_again_before_executing(world: World, monkeypatch):
    ...
    def racing(self, parts, plans, trace, record):
        if not calls:   # the first time through: the world changes between understanding and executing
            calls.append(1)
            self.world.store.create("Table", "table-99", {"number": 99, "capacity": 2, "zone": "indoor"}, self.world.root / "tables" / "99.md")
