...
def _list_bookkeeping_models(session: Session) -> None:
    for internal in ("MoveDoc", "AnchorDoc", "ProjectionDoc"):
        if internal not in session.lex.models:
            session.lex.models.append(internal)
...
def test_i_an_unknown_word_never_offers_the_ledger_or_pron_bookkeeping(world: World):
    ...
    _list_bookkeeping_models(fresh)
