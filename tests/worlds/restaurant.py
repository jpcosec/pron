...
TABLES = [(3, 4, "indoor"), (5, 4, "indoor"), (12, 6, "terrace"), (14, 8, "terrace"), (20, 2, "terrace")]
STATES = ["pending", "confirmed", "seated", "cancelled"]
TRANSITIONS = [("pending", "confirmed", "party_size <= 8"), ("pending", "cancelled", ""), ("confirmed", "seated", ""), ("confirmed", "cancelled", "")]

RELATION_TYPES = [
    {"name": "booked_by", "cardinality": "many_to_one", "axis": "WHAT", "source_types": ["Reservation"], "target_types": ["Client"], "condition": "",
     "description": "Who a reservation belongs to. Every reservation is booked by exactly one client."},
    {"name": "assigned_to", "cardinality": "many_to_one", "axis": "WHERE", "source_types": ["Reservation"], "target_types": ["Table"], "condition": "capacity >= {party_size}",
     "description": "Which table a reservation gets. The table must seat everyone."},
    {"name": "transitions_to", "cardinality": "many_to_many", "axis": "WHEN", "source_types": ["State"], "target_types": ["State"], "condition": "",
     "description": "An allowed move between two states of a status field."},
]

PROJECTION = {
    "name": "all", "stores": ["local"], "models": ["Client", "Table", "Reservation", "State"],
    ...
    "aliases": ["all"],
    "naming": {"Client": "client-{name}", "Reservation": "reservation-{date}-{booked_by.name}", "RelationDoc": "{relation_type}--{source_id}--{target_id}"},
    "display": {"Client": "{name}", "Table": "table {number}", "Reservation": "{date} {time}, {party_size} people, table {assigned_to.number}, {status}"},
    "key": {"Table": "number"},
...
ALIASES = [
    ...
    ("book", ["book her", "book him", "book them", "make a reservation for"], "compose", "create a reservation for someone and put it at a table",
...
def build_restaurant(base: Path, refresh: bool = True) -> World:
    ...
    s = world.store
    for n in STATES:
        s.create("State", f"state-reservation-{n}", {"machine": "Reservation.status", "name": n, "description": f"The reservation is {n}."}, root / "states" / f"{n}.md")
    for number, cap, zone in TABLES:
        s.create("Table", f"table-{number}", {"number": number, "capacity": cap, "zone": zone}, root / "tables" / f"{number}.md")
    s.create("Client", "client-ana-perez", {"name": "Ana Pérez", "phone": "9 1111 0000", "notes": ""}, root / "clients" / "ana-perez.md")
    s.create("Client", "client-luis-soto", {"name": "Luis Soto", "phone": "9 2222 0000", "notes": ""}, root / "clients" / "luis-soto.md")
    s.create("Reservation", "reservation-2026-09-11-luis-soto", {"date": "2026-09-11", "time": "20:00", "party_size": 4, "status": "pending", "notes": ""}, root / "reservations" / "2026-09-11-luis-soto.md")
    for rt in RELATION_TYPES:
        s.create("RelationTypeDoc", f"rt-{rt['name']}", {"title": rt["name"], "direction": "directed", **rt}, root / "relations" / "types" / f"{rt['name']}.md")
    for src, tgt, cond in TRANSITIONS:
        name = f"transitions_to--State:state-reservation-{src}--State:state-reservation-{tgt}"
        s.create("RelationDoc", name, {"title": f"{src} transitions_to {tgt}", "source_id": f"State:state-reservation-{src}", "target_id": f"State:state-reservation-{tgt}",
                                       "relation_type": "transitions_to", "condition": cond, "notes": ""}, root / "relations" / f"{name}.md")
    for rel, src, tgt in (("booked_by", "Reservation:reservation-2026-09-11-luis-soto", "Client:client-luis-soto"), ("assigned_to", "Reservation:reservation-2026-09-11-luis-soto", "Table:table-3")):
        name = f"{rel}--{src}--{tgt}"
        s.create("RelationDoc", name, {"title": name, "source_id": src, "target_id": tgt, "relation_type": rel, "condition": "", "notes": ""}, root / "relations" / f"{name}.md")
    s.create("ProjectionDoc", "projection-all", PROJECTION, root / "knowledge" / "projections" / "all.md")
    for symbol, forms, ref, motive, steps in ALIASES:
        slug = symbol.replace(" ", "-").lower()
        s.create("AnchorDoc", f"anchor-{slug}", {"symbol": symbol, "forms": forms, "ref": ref, "steps": steps, "motive": motive}, root / "knowledge" / "anchors" / f"{slug}.md")
    if refresh:
        world.refresh()
    return world
