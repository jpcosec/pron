...
Los vectores del léxico se calculan una vez por `hash_mundo` y se guardan en `.pron/lexicon.<hash_mundo>.json` en la raíz del mundo: un artefacto derivado, ignorado por git, reconstruible, fuera del store de sldb porque no es un documento. Umbral de sugerencia y cantidad de cercanos (por defecto tres) van en el `ProjectionDoc`, campo `matching`, para que un mundo estricto sugiera menos.
...
Mientras no exista, pron construye y prueba los pasos 1 a 5 sin kgdb: mundo, léxico, sustantivos, ledger, verbos de acción. Los verbos transitivos leídos responden "el grafo no está disponible en este mundo" y los afirmados escriben el `RelationDoc` igual, porque eso es sldb. pron **no** ensambla aristas por su cuenta mientras tanto, ni llamando a `assemble_authored_graph` desde su proyector: la regla 4 del índice vale también en el intervalo.
...
Además, cada escritura por campo compara el `hash_c` del documento leído en el paso 4 con el actual antes de escribir; si difiere, esa escritura no se hace y se reporta con el valor que encontró. Es la única protección; no hay bloqueo de documentos.
