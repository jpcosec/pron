...
- en proceso, `world.payload(model, name, store=None) -> dict`, una copia del payload extraído por sldb; `StoreError` si no existe. Con el id en la mano es `world.store.payload(DocId.parse("A:Model:doc"))`: el store nombra un documento por su `DocId` (`pron.world.doc_id`: store, modelo, nombre; sin store es el local), nunca por sus partes sueltas. **No pasa por ninguna proyección**: lee del store lo que se le pida. El runtime autoriza la referencia antes de leerla.
...
`world.store` es la puerta a sldb entera (`find(scope, where)`, `list`, `get`, `glob`, `schema`, `matches`, y las escrituras `create`, `update_field`, `replace`, `append`, `remove_field`, `clean`, `untrack`), pero un runtime que escribe por ahí está escribiendo por debajo de pron: sin verificación de verbos, sin prevalidación, sin `MoveDoc`. Una superficie como `graph_ui` escribe datos por formas (13), como el SHRDLU, para que sus gestos queden verificados y registrados como movimientos; por aquí queda lo que no tiene forma, como la edición de esquema del párrafo siguiente. Toda operación sobre un documento toma su `DocId` primero (`create(doc_id, payload, path)`, `update_field(doc_id, field, value)`, `untrack(doc_id)`…). `replace(doc_id, payload)` es la misma escritura que `update_field`, íntegra en vez de por ruta punteada, para un runtime que ya construyó el payload completo.

Editar el contrato de un modelo ya registrado es la misma clase de escritura por debajo de pron, y es la única puerta para eso: `model_catalog()` lista los modelos con versión y conteo de documentos; `model_detail(name)` es el modelo como lo arma `models show`; `model_template_edit(name, content)`, `model_fields_add(name, field, type, description, default)` y `model_fields_remove(name, field)` escriben el draft `.py.temp` que sldb mantiene junto al módulo del modelo, sin tocar el modelo activo; `model_validate_draft(name)` valida ese draft (o el modelo activo si no hay draft) sin instalarlo; `model_promote(name)` lo instala, reindexa y sube la versión. Es edición de esquema, no de datos: ningún `MoveDoc` la registra, igual que el resto de este párrafo.
