...
- `TheoremDoc`: una regla del mundo —patrón y cuerpo— con la que se prueban las metas de 13
  (ver 03 §Reglas).
...
## Un mundo en varios stores

Un store de sldb puede enlazar otros (`sldb stores add`), y una proyección dice en `stores` cuáles entran. Así un **daemon** de pron abre un solo store, el suyo, con los stores de los nodos enlazados, y el mundo de cada nodo es una proyección sobre su propio store: `stores: [local]` leída desde el store del nodo significa ese store. Una sesión tiene un **hogar**, el store cuyo mundo es: sus proyecciones se leen de ahí, sus sustantivos se buscan en los stores de la proyección con una consulta por store (`A:st.{Reserva+}` para el store enlazado como `A`, 02), y escribe en el **primero** de esos stores: documentos nuevos, `RelationDoc` (03) y su ledger (07). Un documento de un store enlazado se nombra `A:Modelo:doc`; uno del store propio, `Modelo:doc`. Un nodo a solas abre su store y no sabe del daemon; a través del daemon habla el mismo lenguaje sobre los mismos documentos, porque por abajo son los mismos archivos. Un orquestador tiene una proyección del store del daemon con `stores: [local, A, B]` y ve los tres; eso es distinto de hablarle a otro mundo, que va por su interfaz.
