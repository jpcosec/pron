...
class NounPhrase:
    ...
    complements: list[Any] = field(default_factory=list)  # after "of" / genitive: a run of proper-name tokens (list[str]) or a nested NounPhrase
    ...
    def describe(self) -> str:
        ...
        comps = " ".join("of " + (c.describe() if isinstance(c, NounPhrase) else " ".join(c)) for c in self.complements)
...
def _phrase_at(items: list[Item], i: int, lex: Lexicon) -> tuple[NounPhrase | None, int]:
    ...
    if it.kind == "unknown":
        k = i
        while k < len(items) and items[k].kind in ("unknown", "number") and not items[k].text.endswith("'s"):
            k += 1
        if k < len(items) and items[k].kind == "unknown" and items[k].text.endswith("'s") and k + 1 < len(items) and _head_model(items[k + 1]) is not None:
    ...
    if genitive:
        np.complements.append(genitive); np.items = items[start:i] + [it]
    for adj in pre:
...
def _modifier(items: list[Item], j: int, np: NounPhrase, lex: Lexicon) -> int:
    ...
    if it.kind == "unknown" and it.text.lower() in ("for", "on", "at") and j + 1 < len(items) and items[j + 1].kind == "literal" and items[j + 1].meta.get("kind") in ("date", "time"):
        used = _dated(items[j + 1], np, lex)
        if used:
            np.items.append(it)   # the preposition belongs to the phrase too
        return 1 + used if used else 0
    if it.kind == "unknown" or (it.kind == "literal" and not it.meta.get("marker") and not it.meta.get("kind")):
    ...
    if it.kind == "of" and j + 1 < len(items):
        nxt = items[j + 1]
        if nxt.kind == "word" and _head_model(nxt) is None and _word_modifier(nxt, items[j + 2:], np, lex):
            np.items.append(it)   # "of pron": a value of a field of the head
            return 2
        if nxt.kind == "det" or _head_model(nxt) is not None:
            inner, used = _phrase_at(items, j + 1, lex)   # "of the reservations of Luis": a nested phrase
            if inner is not None:
                np.complements.append(inner); np.items += [it, *inner.items]
                return 1 + used
        run = _proper_run(items, j + 1)
        if run:
            np.complements.append([x.text for x in run]); np.items += [it, *run]
            return 1 + len(run)
        return 0
    if it.kind != "word":
        return 0
...
def _proper_run(items: list[Item], j: int) -> list[Item]:
    ...
    while j < len(items):
        it = items[j]
        if it.kind in ("unknown", "number") or (it.kind == "literal" and not it.meta.get("marker") and not it.meta.get("kind")):
            if it.kind == "unknown" and it.text.lower() in PREDICATE_STOP | {"for", "on", "at"}:
                break
            run.append(it); j += 1
        else:
            break
...
def _dated(it: Item, np: NounPhrase, lex: Lexicon) -> int:
    ...
    for f in lex.world.store.schema(np.model):
        if f["name"] == kind or f["kind"] == kind:
            np.predicates.append(f'{f["name"]} = "{it.meta["value"]}"'); np.items.append(it)
            return 1
    return 0
