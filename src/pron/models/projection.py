...
""".strip()
    ...
    name: str = Field(description="Projection name; the document is named projection-<name>.")
    stores: list[str] = Field(default_factory=lambda: ["local"], description="Stores that enter: 'local' and names of linked stores.")
    models: list[str] = Field(default_factory=list, description="Models that can be named; {Model+} families included. Empty means every model of the store.")
    ...
    actions: list[str] = Field(default_factory=list, description="Kernel verbs allowed: create, change, add, clean, remove, forget, refresh, undo. Empty means none.")
    aliases: list[str] = Field(default_factory=lambda: ["all"], description="Alias documents that enter, by symbol; 'all' means every AnchorDoc.")
    naming: dict[str, str] = Field(default_factory=dict, description="Per model, how a new document is named, e.g. 'client-{name}'; without a rule pron asks.")
