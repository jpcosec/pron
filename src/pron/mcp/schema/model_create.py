...
class ModelCreate:
    ...
    def __call__(self, name: str, fields: list, template: str | None = None, base: str | None = None, confirm: bool = False) -> dict[str, Any]:
        try:
        ...
        plan = {"model_ref": self._ref(source), "path": str(self._path(source)), "module": text, "check": check}
        if not check["ok"]:
            return {"text": f"The module of {name} does not work; nothing was written.", "outcome": "error", "written": False, **plan}
        if not confirm:
            return {"text": f"Would create model {name}. Send confirm: true to write it.", "outcome": "preview", "written": False, **plan}
        return self._register(source, name, fields, template, base, plan)

    def _source(self, name: str, fields: list, template: str | None, base: str | None) -> tuple[ModelSource, str | None]:
        ...
        return ModelSource.of(name, fields, template, (cls.__module__, cls.__name__)), head

    def _register(self, source: ModelSource, name: str, fields: list, template: str | None, base: str | None, plan: dict[str, Any]) -> dict[str, Any]:
        try:
            _sldb_create_model(self.world.store.sp, name, fields, template=template, base=base, target_package=f"{PACKAGE}.{self.package}", pythonpath=self.world.store.pythonpath)
        except SLDBModelError as exc:
            return {"text": f"sldb did not create {name}: {exc}", "outcome": "error", "written": False}
        self.world.refresh()
        return {"text": f"Created model {name}.", "outcome": "unico", "written": True, **plan}

    def _path(self, source: ModelSource) -> Path:
        return Path(self.world.store.pythonpath) / PACKAGE / self.package / f"{source.snake}.py"
