...
simulation, real execution, and undo, so a verb's semantics live in one place: the
kernel's dry run and undo, pre-validation and execution all dispatch to it. `create` and
`assert` stay outside this protocol: their shapes (no export_id yet; a RelationDoc write)
don't fit the same signature, and `Kernel.undo` inverts them together.
"""
